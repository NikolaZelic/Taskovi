<template>
  <aside id="sidebar">
    ...
    {"fas fa-angle-double-left":!sidebarCollapsed}]'>
      ...
      <div class="tabs">
        <button v-for="( tab, index ) in tabs" v-if="index === 0 || companyID !== undefined" :key="index" :title="tab.name" class="tablinks"
          :class="[{active:currentTabIndex === index}, tab.icon]" @click="getTabData(currentTabIndex = index)" :disabled="tab.disabled">
        </button>
      </div>
